...
make_board <- function() {
  for(i in seq(margin + sz, margin + 2*sz, by=sz)) {
    cnv_lines(cnv,
              c(margin, width-margin),
              c(i, i)
              )
    cnv_lines(cnv, c(i, i), c(margin, width-margin))
  }
}
...
##' if so notify, block handlers and return TRUE;
##' else return FALSE
